# ballwall：吊り下げボールを壁のすき間に出し入れするクレーン（研究コード）

軌道最適化 + 時変 LQR による Python での再現と，2 回の独立検証。コマンドはすべて**リポジトリのルートで**実行する。
...
運動方程式の立式から検証までを解説する YouTube 動画（7:48）は [`explainer/`](../explainer/) で作る。作り方はそちらの README を参照。
...
paper/research/  論文（main.tex）、2 ページの要約（x_summary.tex）、図を作るスクリプト
scripts/         docs/ の研究分（図・論文・動画）を作り直すスクリプト
```

リポジトリ全体（ゲーム・動画・もう1本の論文）の構成はルートの [README](../README.md) を参照。
