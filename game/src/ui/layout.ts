...
//   deck          force bar, mini rail, control surface (the rest)
// Layout contract for the renderer: `hudTop` is the bottom of the HUD (band + status row + panel) and the play area
// is [hudTop, scene.y + scene.h]. Its height is sized from the width so that a 2.1-2.3 m window fills it with the
// gantry beam right under the HUD; there is no empty paper above the gantry any more (D10).
...
/** tall: the deck never gets less than max(DECK_MIN_PX, 34 % of the height) nor (when avoidable) more than 46 %. */
export const DECK_MIN_PX = 200;
export const DECK_MIN_FRAC = 0.34;
/**
 * tall, short screens (iPhone SE Safari 375x548, 360x560): the deck gives up to 24 px of that minimum (down to
...
/** Deck contents from the top (§3.3): force bar, mini rail, then the control surface. */
export const DECK_FORCE_H = 8;
export const DECK_RAIL_H = 72;
...
export function computeLayout(w: number, h: number, dpr: number, safeTop = 0): Layout {
  ...
  if (H / W >= 1.25) {
    ...
    const sceneH = Math.min(H - 1, hudTop + play);
    return {
